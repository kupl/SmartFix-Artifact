...
let contractFileBase = path.basename(contractFile)
let outdir = `${process.argv[4]}`
let fixedFile = outdir === undefined ? `fixed/fixed_${contractFileBase}` : `${outdir}/fixed/fixed_${contractFileBase}`
let jsonFile = `${outdir}/json/${contractFileBase}.json`

let solc = process.argv[3] === undefined ? 'solc' : `solc_${process.argv[3]}`
let mainContract = process.argv[5]
// console.log(solc)
...
const { attributes: { name } } = children.find(({attributes: { name }}) => name == mainContract)
