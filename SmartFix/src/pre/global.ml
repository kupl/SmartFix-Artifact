...
= fun callee stmt ->
  (* internal/priviate, view/pure functions, and functions from other contracts are considered to be harmless in terms of deadcode *)
  (* as they will not be annotated with NR *)
...
= fun global f stmt ->
  ...
  | Call (lvop,e,args,ethop,gasop,loc) when is_undef_call global.fmap stmt -> false
...
= fun global f ->
  ...
  (* let _ = print_endline (to_string_fkey (get_fkey f) ^ " -> " ^ Vocab.string_of_set to_string_fkey reachable) in *)
...
= fun global var ->
  let _ = assert (is_address_kind (get_type_var var)) in
...
= fun exp ->
  ...
  | IndexRangeAccess (base,startop,finop,_) ->
    ...
    let set2 = match startop with Some e -> hc_exp e | None -> BigIntSet.empty in
    let set3 = match finop with Some e -> hc_exp e | None -> BigIntSet.empty in
    BigIntSet.union set1 (BigIntSet.union set2 set3)
  | TypeInfo _ -> BigIntSet.empty
...
= fun p slines ->
  ...
  let call_edges = CallGraph.collect_call_edges ~ignore_extern:true ~inlined_cfg:false cnames fmap p in
