...
let edges_n ~ignore_extern : id list -> FuncMap.t -> func -> Node.t -> edge BatSet.t -> edge BatSet.t
= fun cnames fmap curf node edges ->
  ...
  | Call (lvop,e,args,ethop,gasop,loc) (* built-in functions *)
  ...
  | Call _ when is_external_call stmt ->
    if ignore_extern then edges
      ...
      let funcs = List.filter (fun f -> (get_finfo f).scope_s= !Options.main_contract) funcs in
      let funcs = List.filter (fun f -> (is_public_func f || is_external_func f) && not (is_constructor f)) funcs in
      List.fold_left (fun acc callee ->
        BatSet.add (get_fkey curf, get_fkey callee) acc
      ) edges funcs

  | Call (_,e,args,_,_,loc) -> (* static or object calls *)
...
let rec edges_s ~ignore_extern : id list -> FuncMap.t -> func -> stmt -> edge BatSet.t -> edge BatSet.t
= fun cnames fmap curf stmt edges ->
  ...
  | Skip -> edges
  | If (e,s1,s2,i) -> edges |> edges_s ~ignore_extern cnames fmap curf s1 |> edges_s ~ignore_extern cnames fmap curf s2
  | While (e,s) -> edges_s ~ignore_extern cnames fmap curf s edges
...
let edges_f ~ignore_extern ?(inlined_cfg=true) : id list -> FuncMap.t -> func -> edge BatSet.t -> edge BatSet.t
= fun cnames fmap f edges ->
  if inlined_cfg then
    let nodes = nodes_of (get_cfg f) in
    list_fold (edges_n ~ignore_extern cnames fmap f) nodes edges
  else
    edges_s ~ignore_extern cnames fmap f (get_body f) edges

let edges_c ~ignore_extern ?(inlined_cfg=true) : id list -> FuncMap.t -> contract -> edge BatSet.t -> edge BatSet.t
= fun cnames fmap c edges ->
  list_fold (edges_f ~ignore_extern ~inlined_cfg cnames fmap) (get_funcs c) edges

let edges_p ~ignore_extern ?(inlined_cfg=true) : id list -> FuncMap.t -> pgm -> edge BatSet.t
= fun cnames fmap p ->
  list_fold (edges_c ~ignore_extern ~inlined_cfg cnames fmap) p BatSet.empty

let collect_call_edges ~ignore_extern ?(inlined_cfg=true) : id list -> FuncMap.t -> pgm -> edge BatSet.t
= fun cnames fmap p ->
...
= fun cnames fmap p -> transitive_callees (init_callees p) (collect_call_edges ~ignore_extern:true cnames fmap p)
...
= fun p ->
  ...
  print_endline ("[INFO] all funcs : " ^ string_of_int (BatSet.cardinal all));
  print_endline ("[INFO] reachable : " ^ string_of_int (BatSet.cardinal reachable));
