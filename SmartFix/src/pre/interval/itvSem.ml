...
= fun exp mem ->
  ...
  | TypeInfo (typ,x,einfo) ->
    (match typ,x with
     | EType (Contract cid), "name" -> (Itv.top, GTaint.bot, BTaint.bot)
     | EType (Contract cid), "interfaceId" -> (Itv.top, GTaint.bot, BTaint.bot)
     | EType (UInt n), "max" -> (Itv.top, GTaint.bot, BTaint.bot)
     | EType (UInt n), "min" -> (Itv (V zero, V zero), GTaint.bot, BTaint.bot)
...
= fun lv ->
  ...
  | MemberAccess (Cast (_, Lv lv),x,xinfo,_) -> loc_of lv
...
= fun global main_name func node mem ->
  ...
  | Call (lvop,e,args,_,_,loc) when FuncMap.is_internal_call global.fmap global.cnames stmt ->
    let callee = get_callee_of_intcall global func stmt in
    handle_one_callee callee (lvop,e,args) mem
    (* handle_fcall global func (lvop,e,args) mem *)

  | Call _ when is_external_call stmt -> mem

  | Call (lvop,e,args,_,_,loc) -> (* object call: since it may be abstract function, over-approximate it *)
    (match lvop with
     | None -> mem
     | Some (Tuple (eops,_)) ->
       List.fold_left (fun acc eop ->
