...
= fun my_typ comp_typ ->
  if my_typ = comp_typ then true
    ...
    (match my_typ, comp_typ with
     ...
     | ConstString, EType (Contract _) -> false
     ...
     | EType (Enum id1), EType (Enum id2) -> BatString.equal id1 id2
     | EType (Enum _), EType Address -> false
     | Struct _, EType Address -> false
     | EType (Contract c1), EType (Contract c2) -> true
...
let find2 (fid,arg_typs) map =
  BatMap.foldi (fun (_,fid',typs') v acc ->
...
let is_undef exp arg_typs fmap =
  match exp with
  | Lv (MemberAccess (e,fname,_,_)) ->
    (* let fs = find2 (fname, arg_typs) fmap in *)
    if is_contract (get_type_exp e) then
      (* 'fs' can be the empty set, e.g., getter function for mappings *)
      (* let _ = assert (not (BatSet.is_empty fs)) in *)
      false
    else
      (* let _ = assert (BatSet.is_empty fs) in *) (* e.g., abi.encode *)
      true
  | Lv (Var (fname,_)) ->
    let fs = find2 (fname, arg_typs) fmap in
    BatSet.is_empty fs
  | _ -> failwith ("is_undef : " ^ to_string_exp exp)

let is_undef_call : fmap -> stmt -> bool
= fun fmap stmt ->
  match stmt with
  | _ when is_external_call stmt -> false
  | Call (lvop,e,args,ethop,gasop,loc) when is_undef e (List.map get_type_exp args) fmap -> true
  | _ -> false

let is_internal_call : fmap -> id list -> stmt -> bool
= fun fmap cnames stmt ->
  match stmt with
  | _ when is_external_call stmt -> false
  | Call (lvop,e,args,ethop,gasop,loc) when is_undef_call fmap stmt -> false
  | Call (_,Lv (Var (f,_)),_,_,_,_) -> true
  | Call (_,Lv (MemberAccess (Lv (Var (x,_)),_,_,_)),_,_,_,_) when List.mem x cnames -> true
  | _ -> false

let is_internal_call_node : fmap -> id list -> node -> cfg -> bool
