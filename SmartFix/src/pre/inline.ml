...
= fun gvars cnames fmap caller n g ->
  ...
  match stmt with
  | Call (lvop,e,args,_,_,loc)
    when FuncMap.is_undef_call fmap stmt -> (* built-in functions *)
    (false, g)

  | Call (lvop,Lv (Var ("@extern",_)),args,_,_,loc) when (get_fname caller) = "@extern" ->
  ...
  | Call (lvop,e,args,_,_,loc) when is_internal_call fmap cnames stmt ->
    ...
    let excessive = size_of (get_cfg callee) > 5 || has_loop (get_cfg callee) || BatString.starts_with (get_fname callee) "oraclize_" in
...
let is_target_node cnames fmap n g =
  let stmt = find_stmt n g in
  match stmt with
  | Call (lvop,e,args,_,_,loc) when FuncMap.is_undef_call fmap stmt -> false
