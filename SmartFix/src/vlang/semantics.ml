...
= fun lv ->
  ...
  | MemberAccess (e,"balance",info,_) ->
    ...
    let _ = assert (is_address_kind (get_type_exp e)) in
...
= fun exp ->
  ...
  | Cast (EType Address, Lv (Var ("this",_))) -> VVar this_addr
    ...
    (* specifics are handled in z3Interface. *)
  | Cast (typ,e) -> VCast (typ, convert_aexp e)
  | True -> VCond VTrue
  | False -> VCond VFalse
  | UnOp (uop,e,typ) ->
    (match uop with
  ...
  | TypeInfo (typ,x,einfo) ->
    (match typ,x with
     | EType (Contract cid), "name" -> VVar ("@"^cid, ConstString)
     ...
     | EType (UInt n), "min" -> VCast (EType (UInt n), VInt BatBig_int.zero)
...
= fun smap bvars vexp ->
  ...
  | EType etyp ->
    let vf =
      (match etyp with
       | Address | AddressPayable -> VBinRel (VEq, vexp, VInt BatBig_int.zero)
       | Contract cid -> VBinRel (VEq, vexp, VCast (EType (Contract cid), VInt BatBig_int.zero))
  ...
  | Void ->
    let _ = print_endline (to_string_vexp vexp ^ " : " ^ to_string_typ (get_typ_vexp vexp)) in
    assert false
...
let handle_undefs global (lvop,exp,args) (ethop,gasop) loc (vf,qs) =
  ...
  | Lv (MemberAccess (e,f,_,_)) when is_address_kind (get_type_exp e) ->
...
= fun global curf node (vf,qs) ->
  let ftyp_params = List.filter (fun v -> is_func_typ (snd v)) (get_param_vars curf) in
  ...
  | Call (lvop,Lv (Var (v,vinfo)),args,ethop,gasop,loc)
    when List.mem (v,vinfo.vtyp) ftyp_params -> (* function pointers *)
    (match lvop with
     | None -> (vf,qs)
     | Some lv ->
       let def = BatSet.to_list (FuncDefUse.get_def_set_lv lv) in
  ...
  | Call (lvop,e,args,ethop,gasop,loc) when is_internal_call global.fmap global.cnames stmt -> assert false
  | Call _ when is_external_call stmt ->
    if !Options.mode = "exploit" then (VFalse, qs)
    else assert false

  | Call (lvop,e,args,ethop,gasop,loc) -> (* external calls *)
