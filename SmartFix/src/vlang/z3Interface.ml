...
= fun typ ->
  ...
  | Struct s -> Z3.Sort.mk_uninterpreted_s !ctx (to_string_typ typ)
  | TupleType _ -> failwith "typ_to_sort: tuple types should not be queried as a whole."
  | FuncType _ -> failwith "typ_to_sort: FuncType"
  | Void -> failwith "typ_to_sort: Void"

and etyp_to_sort : elem_typ -> Z3.Sort.sort
= fun etyp ->
  match etyp with
  | Address | AddressPayable -> Z3.BitVector.mk_sort !ctx 160
...
= fun my_typ comp_typ z3exp ->
  if my_typ = comp_typ then z3exp
   ...
   (match my_typ, comp_typ with
    ...
    | ConstInt, EType (Bytes n) -> (* e.g., bytes16(17) *)
      Z3.Arithmetic.Integer.mk_int2bv !ctx (n*8) z3exp
    (* shortening case *)
    | EType (UInt n1), EType (UInt n2) when n1>n2 ->
      Z3.BitVector.mk_extract !ctx (n2-1) 0 z3exp
    (* other cases *)
    | EType (Bytes n1), EType (UInt n2)
      when n1*8<=n2 -> (* syntax disallowed since solc 0.5.0 *)
      Z3.BitVector.mk_zero_ext !ctx (n2-(n1*8)) z3exp
    | EType Address, EType (Contract s) ->
      Z3.Expr.mk_fresh_const !ctx (freshvar ()) (typ_to_sort comp_typ)
    | EType (Contract s), EType Address ->
    ...
    | EType (Bytes n1), EType (Bytes n2) ->
      ...
      if n1=n2 then z3exp
        ...
        Z3.BitVector.mk_extract !ctx (n1*8-1) (n1*8-n2*8) z3exp
      else if n1<n2 then
        let zeros = Z3.BitVector.mk_numeral !ctx "0" ((n2-n1)*8) in
        Z3.BitVector.mk_concat !ctx z3exp zeros
      else assert false
    | ConstInt, EType (Contract s) -> Z3.Expr.mk_fresh_const !ctx (freshvar ()) (typ_to_sort comp_typ)
    ...
    | EType (UInt n1), EType (Bytes n2) -> (* disallowed since solc 0.5.0 *)
    ...
    | EType (Contract id), EType (UInt n) -> Z3.Expr.mk_fresh_const !ctx (freshvar ()) (typ_to_sort comp_typ)
    | EType (Contract id1), EType (Contract id2) -> Z3.Expr.mk_fresh_const !ctx (freshvar ()) (typ_to_sort comp_typ)
    ...
    | _ -> (* for unknown cases, simply assign fresh variables *)
      Z3.Expr.mk_fresh_const !ctx (freshvar ()) (typ_to_sort comp_typ))
...
= fun vf ->
  ...
  | Label _ -> failwith "z3interface: Label"
  ...
  if t1=t2 then t1
   ...
   (match t1,t2 with
    | ConstInt, EType (UInt n) ->
      (* In 'get_bigint_v e', assume constants in 'e' are already folded *)
...
and cast_ve ve =
  ...
  | VBinOp (vbop,e1,e2,t) when vbop=VShiftL || vbop=VShiftR ->
    ...
    (* Also, the invariant 't=t1' does not hold. Different from the official Solidity document, '3<<n' (where 'n' is uint type) has 'uint' type. *)
    (* let _ = assert (t = t1) in *)
    (* In conclusion, simply obey types produeced by solc. *)
