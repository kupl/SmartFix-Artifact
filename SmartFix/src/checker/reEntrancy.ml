...
= fun global vf path g node ->
  ...
  let b4 = FuncMap.is_internal_call_node global.fmap global.cnames (BatList.last (snd path)) g in
  ...
  | Call (lvop,Lv (MemberAccess (e,fname,_,_)),args,_,_,loc)
    when is_address_kind (get_type_exp e) && List.mem fname ["transfer";"send"] && b1 ->
