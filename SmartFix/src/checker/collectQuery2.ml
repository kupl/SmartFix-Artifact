...
let collect ?(reg_check=false) ?(extern_ctx=false) : Global.t -> cfg -> vformula -> Path.t -> Node.t -> query list
= fun global g vf path node ->
  if reg_check then Regression.collect_queries global vf path (find_stmt node g)
  else
