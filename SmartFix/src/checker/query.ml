...
let ts_kind = to_string_kind_simple
...
type qid = kind * int * string (* line in the original source code *)

let compare_qid : qid -> qid -> int
...
let equal_qid qid1 qid2 = (compare_qid qid1 qid2 = 0)

let get_qid : query -> qid
= fun q -> (q.kind, q.qloc, to_string_origin ~report:true q.org_q)

let to_string_standard_qid (k,l,s) =
...
let to_string_qid (k,l,s) =
  if l>0 then "line " ^ string_of_int l ^ ", " ^ s
  else to_string_standard_qid (k,l,s)
