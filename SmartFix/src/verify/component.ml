...
let is_addr_kind_mapping_typ : typ -> bool
= fun typ ->
  match typ with
  | Mapping (Address, t) when is_address_kind t -> true
  | _ -> false
...
= fun global exp comps ->
  ...
  | True | False | ETypeName _ | IndexRangeAccess _ | TypeInfo _ -> comps
...
= fun global lv comps ->
  ...
  | Var (x,xinfo) ->
    let t = xinfo.vtyp in
    { comps with
      mapvars = if is_usual_mapping t then BatSet.add (x,t) comps.mapvars
                else comps.mapvars;
      ivars = if (is_uintkind t || is_sintkind t) then BatSet.add (x,t) comps.ivars
              else comps.ivars;
      avars = if is_address_kind t && not (is_constant_address global (x,t)) then BatSet.add (x,t) comps.avars
              else comps.avars;
      ...
      bvars = if is_bool t && List.mem (x,t) global.gvars then BatSet.add (x,t) comps.bvars
              else comps.bvars;
    }
...
= fun global node g comps ->
  ...
  | Call (lvop,e,elst,ethop,gasop,_) ->
