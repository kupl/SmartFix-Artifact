...
let tu_each ve tf =
  ...
  match typ  with
  | _ when is_address_kind typ -> VBinRel (VEq, Read (VVar trust_map, ve), tf)
  | Array (t,_) when is_address_kind t ->
...
let is_addr_or_addr_array (_,typ) =
  match typ with
  | _ when is_address_kind typ -> true
  | Array (t,_) when is_address_kind t -> true
  | _ -> false
...
= fun global vf ->
  ...
  | Label (0, VBinRel (VEq, VVar x, _)) ->
    let fields = BatMap.bindings global.smap |> List.map snd in
    let field_names = List.fold_left (fun acc members -> acc @(List.map fst members)) [] fields in
    if List.mem (org x) global.gvars || List.mem (fst (org x)) field_names || fst (org x) = "@Invest_sum" then true
...
= fun vf ->
  ...
  | Label (0, VBinRel (VEq, VVar x, VCond VFalse)) when BatString.starts_with (fst x) "success__@" || BatString.starts_with (fst x) "Param" -> true
...
= fun vf ->
  ...
  | Label (0, VBinRel (VEq, VVar x, VCond VTrue)) when BatString.starts_with (fst x) "success__@"  || BatString.starts_with (fst x) "Param" -> true
  | VBinRel (VEq, VVar x, VCond VTrue)
    (* likely ret param is false at the end *)
    when BatString.starts_with (fst x) "success__@" || BatString.starts_with (fst x) "Param" -> true
  | _ -> false

let known_addition_pattern : vformula -> bool (* focus on common pattern *)
= fun vf ->
  match vf with
  | VBinRel (VGeq, VBinOp (VAdd, e, VVar var, EType (UInt 256)), e')
    when BatString.starts_with (fst var) "_value__@" || BatString.starts_with (fst var) "value__@"
    -> true
  | _ -> false

let ends_with_fexit : Path.t -> bool
= fun path ->
  let bp = Path.get_bp path in
  let last = BatList.last bp in
  is_exit last

let is_query_from_main_contract : query -> bool
= fun q -> (fun (c,_,_) -> c = !Options.main_contract) q.src_f
...
= fun global mem path q ->
  ...
  | IO ->
    ...
    if b then (b,mop)
    else if !Options.alarm_filter && !Options.refined_vcgen && not (g_write global pre)
...
let proven : Query.qid -> QMap.t -> bool
= fun qid qmap ->
  QMap.find qid qmap
...
= fun global mem qs qmap_prev ->
  ...
  BatList.fold_lefti (fun (acc_p, acc_qmap, acc_m, regression_found) i qs_bundle ->
    let (kind,loc,str) = Query.get_qid (snd (List.hd qs_bundle)) in
    ...
    if proven (kind,loc,str) qmap_prev then
      ...
      if PathSet2.is_empty unproven_paths then
        ...
        let reg_found = kind = NO_EFFECT || kind = ASSIGN_CONST || kind = DEAD in
        ...
        (acc_p', acc_qmap', acc_m, regression_found)
  ) (PathSet2.empty, qmap_prev, ModelMap.empty, false) qss
  |> (fun (a,b,c,_) -> (a,b,c))
...
= fun global mem pairs ->
  List.fold_left (fun (acc_p, acc_m, acc_c, failed) (path,vc) ->
    ...
    | false,Some m -> (PathSet2.add path acc_p, acc_m, Component.union comps acc_c, failed + 1)
    | false,None -> (PathSet2.add path acc_p, acc_m, Component.union comps acc_c, failed + 1)
  ) (PathSet2.empty,ModelMap.empty,Component.empty_comps,0) pairs
  |> (fun (a,b,c,d) -> (a,b))
...
= fun qmap ->
  ...
  List.fold_left (fun acc (k, ((stat,_,_) as v)) ->
    match stat with
    | Proven -> QMap.add k v acc
    | UnProven ->
      if List.mem (k,v) unproven then QMap.add k v acc
      else acc
    | Disproven -> assert false
  ) QMap.empty all

(* usual verification routine *)
let do_verify : Global.t -> ItvDom.Mem.t -> PathSet.t -> (QMap.t * QMap.t * InvMap.t * SpecMap.t)
...
let check_regression global mem (invmap,specmap) vpaths =
  ...
  let _ = if !Options.verbose then Profiler.finish "Checking validity of VCs ... " in
  let _ = print_endline "" in
  let _ = reg_check_phase := false in
  reg_qmap

let verify_org : Global.t -> ItvDom.Mem.t -> PathSet2.t -> (QMap.t * QMap.t * InvMap.t * SpecMap.t)
...
let verify_cand : Global.t -> ItvDom.Mem.t -> PathSet2.t -> QMap.t -> (InvMap.t * SpecMap.t) -> (QMap.t * QMap.t * InvMap.t * SpecMap.t)
...
let verify_inv_hold : Global.t -> ItvDom.Mem.t -> PathSet2.t -> (InvMap.t * SpecMap.t) -> bool
= fun global mem vpaths (invmap,specmap) ->
  ...
  let inductive = PathSet2.is_empty unproven_paths in
  inductive

exception Solution_Inv_Not_Hold

let verify_when_solution_found : Global.t -> ItvDom.Mem.t -> PathSet2.t -> PathSet.t -> (QMap.t * QMap.t * InvMap.t * SpecMap.t)
= fun global mem vpaths paths ->
  let (invmap0, specmap0, _, qmap0) = init global vpaths in
  let (success, invmap, specmap) =
    let invdir = WorkspaceRepair.json_report_in !Options.repair_compdir in
  ...
  if not success then (* parsing failed: usual patch verification *)
    ...
    if inductive then (* inductive: proceed *)
      verify_cand global mem vpaths qmap0 (invmap,specmap)
    else
      (* optimization using diff verification optimization *)
      (* - not inductive: simply stop verification *)
      raise Solution_Inv_Not_Hold

let verify_patch : Global.t -> ItvDom.Mem.t -> PathSet2.t -> PathSet.t -> (QMap.t * QMap.t * InvMap.t * SpecMap.t)
= fun global mem vpaths paths ->
  let solution_found =
    let comp_alarms =
      if !Options.repair_compdir = "" then None
      else Some (Report.csv_report_alarms (WorkspaceRepair.vulreport_in !Options.repair_compdir))
    in
    match comp_alarms with None -> false | Some n -> n = 0 in
...
let do_repair_verify : Global.t -> ItvDom.Mem.t -> PathSet.t -> (QMap.t * QMap.t * InvMap.t * SpecMap.t)
= fun global mem paths ->
  verify_start_time := Sys.time();
  iter := 0;
  let global = augment_global paths global in
  let vpaths = augment_paths global paths in
  (* let _ = PathSet2.iter (fun p -> print_endline (Path2.to_string p)) vpaths in
  let _ = assert false in *)
  if !Options.repair_overify then
    verify_org global mem vpaths
  else if !Options.repair_pverify then
    verify_patch global mem vpaths paths
  else assert false


let run : Global.t -> ItvDom.Mem.t -> PathSet.t -> (QMap.t * QMap.t * InvMap.t * SpecMap.t)
= fun global mem paths ->
  if !Options.repair_overify || !Options.repair_pverify then
    let _ = assert !Options.check_reg in
