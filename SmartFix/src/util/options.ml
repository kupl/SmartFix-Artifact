...
let uninterp_nonlinear = ref false

(******************************)
(* Belows are for dev version *)
(******************************)
...
let check_re = ref false
let check_tx = ref false
let check_leak = ref false
let check_erc20 = ref false
let check_reg = ref false
...
let extern_depth = ref 1 (* maximum external call depth *)
...
let repair_tool_timeout = ref 180
let repair_loop_timeout = ref 1800
let repair_no_learn = ref false
let repair_no_diff = ref false
let repair_ksrc = ref ""
let repair_overify = ref false
let repair_pverify = ref false
let repair_compdir = ref ""
let repair_report = ref ""
...
let activate_default_detector_if_unspecified () =
  let b =
    ...
    !check_kill || !check_erc20 || !check_leak ||
    !check_re || !check_tx || !check_reg in
  if b=false then (check_io:=true; check_dz:=true; check_leak:=true; check_kill:=true)
  else ();
  if !repair_overify || !repair_pverify then (check_reg:=true)
...
let print_detector_activation_status () =
  ...
  (if !check_io          then prerr_endline "[CHECKER] Integer Over/Underflows";
   ...
   if !check_re          then prerr_endline "[CHECKER] Re-Entrancy";
   if !check_tx          then prerr_endline "[CHECKER] Improper use of Tx.origin";
   if !check_leak        then prerr_endline "[CHECKER] Ether-Leaking";
   prerr_endline "")

let set_mode s =
  match s with
  | "verify" | "exploit" | "repair" | "bugsynth" -> mode:=s
  | _ -> invalid_arg "improper arguments for run mode. Should be one of {verify, exploit, repair, bugsynth}"

let options =
  [
    ("-input", Arg.String (fun s -> inputfile := s), "input Solidity file");
    ...
    ("-solv", Arg.String (fun s -> solc_ver := s), "specify solidity compiler version, e.g., 0.5.13");
    ...
    ("-uninterp_nonlinear", Arg.Set uninterp_nonlinear, "use uninterpreted funciton for nonlinear exps");

    ("-verbose", Arg.Set verbose, "verbose mode");
    ("-report", Arg.Set mk_report, "produce analysis/repair reports");
    ...
    ("-path", (Arg.Int (fun n -> path := n)), "path enumeration mode");

    ("-re_safety_enforce", Arg.Set re_safety_enforce, "perform reentrancy safety verification even when CEI pattern is obeyed.");
    ...
    ("-edepth", Arg.Int (fun n -> extern_depth:=n), "maximum external function call depth");
    ...
    ("-enforce_init_eth", Arg.Set enforce_init_eth, "do not allow arbitrary setting of contract ethers when ether-leak checker is activated (default: false)");

    (* repair mode *)
    ("-repair_tool_timeout", Arg.Int (fun n -> assert (n>0); repair_tool_timeout := n), "timeout for each analyzer (default: 60s)");
    ("-repair_loop_timeout", Arg.Int (fun n -> assert (n>0); repair_loop_timeout := n), "timeout for repair loop (default: 1800s)");
    ("-repair_no_learn", Arg.Set repair_no_learn, "do not perform online- and offline learning, i.e., search in increasing order");
    ("-repair_no_diff", Arg.Set repair_no_diff, "do not perform differential verification during patch validation (default: false)");
    ("-repair_ksrc", Arg.String (fun s -> repair_ksrc := s), "dir of knowlege source file for transfer learning");
    ("-repair_overify", Arg.Set repair_overify, "do verification for original contract in repair mode");
    ("-repair_pverify", Arg.Set repair_pverify, "do patch verification");
    ("-repair_compdir", Arg.String (fun s -> repair_compdir := s), "comparison target directory for differential verification");
    ("-repair_report", Arg.String (fun s -> repair_report := s), "report file containing fix targets");

    ("-outdir", Arg.String (fun s -> outdir:= s), "directory in which outputs are stored")
  ]
