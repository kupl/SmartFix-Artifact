...
contract CommissionsStudio {
    ...
    isValidQueue(_creator, _queueId)
    {
        ...
        selectedCommission.recipient.transfer(selectedCommission.bid); /* <RE_VUL> */
    ...
    isValidQueue(msg.sender, _queueId)
    {
        ...
        for (uint i = 0; i < _commissionIds.length; i++){
            ...
            // selectedCommission.status = CommissionStatus.accepted; /* [1: move to line 220] */ // first, we change the status of the commission to accepted
            (bool success, ) = payable(msg.sender).call{ value: selectedCommission.bid - feePaid }(""); /* <RE_ENT>, <RE_VUL>, [3: transfer->call.value] */ // next we accept the payment for the commission
